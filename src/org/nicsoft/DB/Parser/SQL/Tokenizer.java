...
import org.nicsoft.DB.Parser.JSON.JSONParser;
...
public class Tokenizer {
	...
	public Tokenizer(String sql) throws Exception {
		...
		for(int i=0; i<sql.length(); i++) {
			...
			if(c == '\'' && singleQuoted && !escape) { // End single quote
				...
				try {
					...
					this.tokens.peekLast().overrideKeyWord(
						KeyWord.JSON_DATA_STRUCTURE
					);
